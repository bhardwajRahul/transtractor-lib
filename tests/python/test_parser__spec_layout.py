...
def test_spec_layout_generates_correct_file():
    """Test that parsing test1.layout generates a spec matching test1_spec.json."""
    parser = Parser()
    ...
    with tempfile.NamedTemporaryFile(
        ...
        parser.spec_layout(str(test_layout), tmp_spec_path)
...
def test_spec_layout_raises_parse_error_without_config():
    ...
    with pytest.raises(ParseError):
        parser.spec_layout(str(test_layout), "dummy_path.json")


def test_spec_layout_raises_parse_error_with_misconfigured_config():
