...
mod tests {
    ...
    fn test_format7_parse() {
        ...
        let ts = fmt.parse("24-03-2023", "", 1);
        assert!(ts.is_some());
        // "24-3-2023" (DD-M-YYYY, single-digit month)
        let ts2 = fmt.parse("24-3-2023", "", 1);
        assert!(ts2.is_some());
        // "24-03-23" (DD-MM-YY)
        let ts3 = fmt.parse("24-03-23", "", 1);
        assert!(ts3.is_some());
        // "24-3-23" (DD-M-YY)
        let ts4 = fmt.parse("24-3-23", "", 1);
        assert!(ts4.is_some());
        // "1-1-2023" (D-M-YYYY, both single-digit)
        let ts5 = fmt.parse("1-1-2023", "", 1);
        assert!(ts5.is_some());
        // "31-12-2023" (December 31)
        let ts6 = fmt.parse("31-12-2023", "", 1);
        assert!(ts6.is_some());
        // Invalid date (February 30)
        let ts7 = fmt.parse("30-02-2023", "", 1);
        assert!(ts7.is_none());
        // Invalid format - wrong separator (slash)
        assert_eq!(fmt.parse("24/03/2023", "", 1), None);
        ...
        assert_eq!(fmt.parse("24-03-3", "", 1), None);
        // Invalid format - too many digits in day
        assert_eq!(fmt.parse("240-03-2023", "", 1), None);
    }
}
