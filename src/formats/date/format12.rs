...
/// Format12: parses YYYY/MM/DD dates like "2023/03/24", "2023/3/24"
pub struct Format12;

impl DateFormat for Format12 {
...
mod tests {
    ...
    #[test]
    fn test_format12_parse() {
        let fmt = Format12;
        // "2023/03/24" (YYYY/MM/DD)
        let ts = fmt.parse("2023/03/24", "", 1);
        assert!(ts.is_some());
        // "2023/3/24" (YYYY/M/DD, single-digit month)
        let ts2 = fmt.parse("2023/3/24", "", 1);
        assert!(ts2.is_some());
        // "2023/03/4" (YYYY/MM/D, single-digit day)
        let ts3 = fmt.parse("2023/03/4", "", 1);
        assert!(ts3.is_some());
        // "2023/3/4" (YYYY/M/D, both single-digit)
        let ts4 = fmt.parse("2023/3/4", "", 1);
        assert!(ts4.is_some());
        // "2023/01/01" (January 1)
        let ts5 = fmt.parse("2023/01/01", "", 1);
        assert!(ts5.is_some());
        // "2023/12/31" (December 31)
        let ts6 = fmt.parse("2023/12/31", "", 1);
        assert!(ts6.is_some());
        // Invalid date (February 30)
        let ts7 = fmt.parse("2023/02/30", "", 1);
        assert!(ts7.is_none());
        // Invalid format - wrong separator (dash)
        assert_eq!(fmt.parse("2023-03-24", "", 1), None);
        // Invalid format - empty string
        assert_eq!(fmt.parse("", "", 1), None);
        // Invalid format - missing parts
        assert_eq!(fmt.parse("2023/03", "", 1), None);
        // Invalid format - two-digit year
        assert_eq!(fmt.parse("23/03/24", "", 1), None);
        // Invalid format - three-digit year
        assert_eq!(fmt.parse("202/03/24", "", 1), None);
        // Invalid format - too many digits in month
        assert_eq!(fmt.parse("2023/003/24", "", 1), None);
    }
}
