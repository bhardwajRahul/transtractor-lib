...
mod tests {
    ...
    fn test_format10_parse() {
        ...
        let ts = fmt.parse("Mar 24", "2023", 2);
        assert!(ts.is_some());
        // "Mar 4" (single-digit day)
        let ts2 = fmt.parse("Mar 4", "2023", 2);
        assert!(ts2.is_some());
        // "March 4" (full month name)
        let ts3 = fmt.parse("March 4", "2023", 2);
        assert!(ts3.is_some());
        // "Jan 1" (January 1)
        let ts4 = fmt.parse("Jan 1", "2023", 2);
        assert!(ts4.is_some());
        // "Dec 31" (December 31)
        let ts5 = fmt.parse("Dec 31", "2023", 2);
        assert!(ts5.is_some());
        // "January 15"
        let ts6 = fmt.parse("January 15", "2023", 2);
        assert!(ts6.is_some());
        // Invalid date (February 30)
        let ts7 = fmt.parse("Feb 30", "2023", 2);
        assert!(ts7.is_none());
        // Invalid format - wrong order (DD MMM)
        // Note: "24 Mar" will match regex but fail parsing
        let ts8 = fmt.parse("24 Mar", "2023", 2);
        assert!(ts8.is_none());
        // Invalid format - empty string
        assert_eq!(fmt.parse("", "2023", 2), None);
        // Invalid format - missing parts
        assert_eq!(fmt.parse("Mar", "2023", 2), None);
        // Invalid format - includes year
        assert_eq!(fmt.parse("Mar 24 2023", "2023", 2), None);
    }
}
